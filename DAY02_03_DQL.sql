/*
    ...
    6. FROM까지는 필수, WHERE부터 선택. GROUP BY와 HAVING은 세트로 다님.
*/
...
SELECT *             -- * : 모든 칼럼을 의미, 실제 개발할때는 사 용 금 지!(왜? 성능이 떨어짐)
  FROM DEPARTMENT_T;
  
SELECT DEPARTMENT_T.DEPT_NO, DEPARTMENT_T.DEPT_NAME, DEPARTMENT_T.DEPT_LOCATION
  FROM DEPARTMENT_T;
...
SELECT DISTINCT DEPT_LOCATION   -- 중복제거 : DISTINCT
  FROM DEPARTMENT_T;
  
SELECT DEPT_LOCATION  -- 동일한 위치는 묶어서 그룹화
  FROM DEPARTMENT_T
 ...
 /*
    ...
    3. 칼럼 IN(값1, 값2)
    4. 칼럼 IS NULL - NULL값을 가지고 있는지 점검하는 조건문
    5. 칼럼 IS NOT NULL - NULL값을 갖고 있지 않은 조건문
    6. 칼럼 LIKE 값 - 저 값이 포함된 것을 찾는 것.
 */ 
...
SELECT *
  FROM DEPARTMENT_T
 WHERE DEPT_LOCATION = '서울'; -- DEPT_LOCATION 칼럼은 UNIQUE하지 않기 때문에 조회 결과는 2개 이상이 가능하다.
